const url = "https://40f0cd47-5b08-4846-815a-154830637e85-00-3kfucwm1ywejb.sisko.replit.dev/iptv/iptv3m3u.js"
fetch(url)
  .then((response) => {
...
const videoFun1 = (m3u8URL1) => {
      const isAndroid = /Android/i.test(navigator.userAgent);
      const networkStreamPlayerURL = `intent://${m3u8URL1}#Intent;scheme=https;type=video/*;package=com.genuine.leone;S.browser_fallback_url=market://details?id=com.genuine.leone.ad;S.title=PLAYIDTV;end`;
    if (isAndroid) {
        const iframe = document.createElement('iframe');
        iframe.style.display = 'none'; 
         iframe.src = networkStreamPlayerURL;
